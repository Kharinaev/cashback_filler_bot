...
4. Create new variant if there is no matching variant.
   For every category return one suitable visual emoji. For an existing category,
   reuse its emoji from the list; for a new category, invent a clear suitable emoji.
...
[
    {
        ...
        "Emoji": string, // exactly one suitable emoji for the category
